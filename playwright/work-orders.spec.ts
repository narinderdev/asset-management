...
const seedAuth = (page: Page) =>
  page.addInitScript(() => {
    ...
    localStorage.setItem('userPermissions', JSON.stringify({ modules: { WORK_ORDER: ['CREATE', 'UPDATE', 'DELETE', 'VIEW'] } }));
  });
...
test.describe('Work Orders', () => {
  ...
  test('creates a work order', async ({ page }) => {
    ...
    await page.locator('input[name="descriptionScope"]').fill('Replace mechanical seal');
